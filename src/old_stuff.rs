...
fn main() {
    App::new()
        ...
        .add_systems(Update, (mouse_input_system, spawn_circle_system, remove_oldest_circle_system)fa)
        .run();
}
...
fn remove_oldest_circle_system(
    mut commands: Commands,
    query: Query<(Entity, &CircleMarker)>,
) {
    // sort circles by spawn_order (oldest first)
    let mut circles: Vec<(Entity, &CircleMarker)> = query.iter().collect();
    circles.sort_by_key(|(_, marker)| marker.spawn_order);

    // remove the oldest circle if any exist
    if let Some((entity, _)) = circles.first() {
